...
class ARSCNViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.loadImageSizeFromUserDefaults()
...
//        sceneView.addGestureRecognizer(panGesture)
    ...
    func setSelectedImage(width: CGFloat, height: CGFloat) {
        ...
        if let planeNode = imageNode {
            ...
            viewModel?.saveImageSizeToUserDefaults(width: width, height: height)
        }
    }
    ...
    // Load image size from UserDefaults
    private func loadImageSizeFromUserDefaults() {
        let userDefaults = UserDefaults.standard
        let width = userDefaults.float(forKey: "imageWidth")
        let height = userDefaults.float(forKey: "imageHeight")
        ...
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { action in
            ...
            if let widthText = alert.textFields?[0].text, let heightText = alert.textFields?[1].text {
                if let width = Double(widthText), let height = Double(heightText) {
                    ...
                    self.setSelectedImage(width: width/3.0, height: height/3.0)
                } else {
